# like main model, but groups some covarriates

test<- FALSE #td: change back
fast <- TRUE
speed.factor <- 1
...
macro$notes <- append(macro$notes,"groups first two main interventions")
...
macro$MCMC_n_adapt <- 4000/speed.factor
macro$MCMC_n_burn <- 12000/speed.factor
macro$MCMC_n <- 15000/speed.factor
macro$MCMC_chains <- 2
macro$MCMC_thin <- 60
...
### merge group1 and drop single interventions
# from(11.03 - 14.03)
group1.list <- c("eventslimited","speechlimited",
                 "bars","education","sports",
                 "restaurants")
# check existence
if(any(!group1.list%in%macro$dummies.interventions))
  stop("Not all interventions exist in data")
# generate new group intervention group1
idat <- idat %>% mutate(
  group1 = rlang::eval_tidy(rlang::parse_expr(paste0(group1.list,collapse = "|"))))
# drop single interventions and add new group
macro$dummies.interventions <- setdiff(
  c("group1",macro$dummies.interventions),
  group1.list)


### merge group2 and drop single interventions
# from 16.03 - 19.03
group2.list <- c("events", "gatheringslimited","shops",
                 "speechMerkel")
# check existence
if(any(!group2.list%in%macro$dummies.interventions))
  stop("Not all interventions exist in data")
# generate new group intervention group2
idat <- idat %>% mutate(
  group2 = rlang::eval_tidy(rlang::parse_expr(paste0(group2.list,collapse = "|"))))
# drop single interventions and add new group
macro$dummies.interventions <- setdiff(
  c("group2",macro$dummies.interventions),
  group2.list)
...
macro$dummies <- c(macro$dummies.intervention,
                   macro$dummies.FE,
                   macro$dummies.factors)
