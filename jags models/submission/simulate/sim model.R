...
load("C:/Users/patricks/Dropbox/R packages/InfSup/jags models/submission/save/submission.RData")
...
### create model path
if(server){
  model_path <- paste0("/home/quamet/patrick/submission/sim ",macro$model_name,".R")
} else {
  model_path <- paste0("./jags models/submission/sim ",macro$model_name, ".R")
}

# data inclusion ----------------------------------------------------------


macro$total_limit <- 20
...
exclude_ids <- data$IdLandkreis%>%unique()
...
data <- data %>%
  filter(!IdLandkreis %in% exclude_ids)
...
iter.all <- mymcmc::extract_length(samps.list)
...
res.all <- data.frame()
sample.cur <- sample(1:iter.all,n.post.draws)
for(iter.cur in sample.cur){
  cat(which(sample.cur == iter.cur),'\n')
  dat.res <- mymcmc::extract_inits_from_mcmc(iter = iter.cur,
    samps.list,vars =
      c("mean_SI|mean_transmission|Rzero|beta_effect|beta_effectxd|i_disp"))
  ...
  i[,,]<-NA
  for(c in 1:dat$nc){
    timing.cur <- (Ti[c]-max_past):Ti[c]
    i[1:min(timing.cur),c,]<-0
    i[timing.cur,c,]<-reports[timing.cur+5,c,]*4
  }
  ...
  dat.combined <- c(dat,dat.sim,dat.res)


  jm <- rjags::jags.model(model_path,
                          data = dat.combined,n.chains = 1,n.adapt = 0)

  # res.sim2 <- rjags::jags.samples(jm,monitor,sim.iter)
  # res.sim <- res.sim2[names(res.sim2)]

  res.cur <- rjags::coda.samples(jm,monitor,sim.iter)

  res.cur <- mymcmc::extract_res(res.cur)%>%
    select(varname,value,V1,V2,V3,.iteration)%>%
    rename(t=V1,c=V2,a=V3)%>%
    mutate(piter=iter.cur)

  res.all <-
    rbind(res.all,res.cur)
}
...
res.all <- res.all %>%
  left_join(
    data.frame(
      name=dimnames(reports)[[2]],c=1:dat$nc)
    ) %>%
  left_join(
    data.frame(
      age=dimnames(reports)[[3]],a=1:dat$na)
  )%>%
  left_join(
    data.frame(
      date=data$date%>%unique(),t=1:dat$nt)
  )

save(res.all,data,dat.combined,file = "./jags models/submission/simulate/simdat.RData")
