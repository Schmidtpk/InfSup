...
macro$notes <- append(macro$notes,"change reporting date")
macro$notes <- append(macro$notes,"change to death")
...
# from "inferring ineffectiveness..." science paper mean (infection to death): N (21.82,æ = 1.01)
# but in Germany reporting date does not imply date of death, but first report to health department. Plot below suggests this is mostly before death
  # rki_new %>% filter(AnzahlTodesfall>0)%>%
  #   mutate(delay = as.numeric(Meldedatum-Refdatum))%>%
  #   filter(abs(delay)<30)%>%pull(delay)%>%hist(breaks=1000)
  # rki_new %>%
  #   mutate(delay = as.numeric(Meldedatum-Refdatum))%>%
  #   filter(abs(delay)<30)%>%pull(delay)%>%hist(breaks=1000)
...
macro$mean_SI_mean <- 10.92
macro$mean_SI_sd <- 0.94
...
data <- data %>%
  ...
  group_by(Meldedatum,name,age,Bundesland)%>%
  summarise(
...
reports <- array(data %>% pull(dead.new),
...
macro$dummies <- c(macro$dummies.interventions, macro$dummies.FE,macro$dummies.factors)
