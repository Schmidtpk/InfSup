    ...
    for(c in 1:nc){
        ...
        for (t in Ti[c]:T[c]){
            for(a in 1:na){
                ...
                r_negbin[t,c,a] = i_disp[a]
                p_negbin[t,c,a] = r_negbin[t,c,a]/(r_negbin[t,c,a]+Es[t,c,a])

                reports[t,c,a] ~ dnegbin(p_negbin[t,c,a], r_negbin[t,c,a])
            }
        }
    }
