...
vec.path <- c(
  ...
  "./jags models/submission/save/robust priorhigh01-02.RData",
  "./jags models/submission/save/robust priorlow01-02.RData"
)
...
res <- all%>%
  ...
  mutate(true = run=="main",
         m = as.factor(m),
         orderby = ifelse(n()<1,-Inf,-mean[true]))%>%
  ungroup()%>%
  mutate(
...
ggplot(res.plot
  ...
  scale_shape_manual('priors',values = c("main" = 4,"less informative"=1,"more informative"=2))+
  theme(
    axis.title.y = element_blank()
  )+
  coord_cartesian(xlim=limitsx)
ggsave("./analysis/tex/plotsnew/res_robustR2_priors.pdf",
      width = 6,height=8)
