...
ggsave("./analysis/tex/plotsnew/res_allR.pdf",
       width = 6,height=8)
...
ggsave("./analysis/tex/plotsnew/resR.pdf",
       width = 4,height=3.5)

show_effects_sample(cov = c(macro$cov,setdiff(macro$dummies,macro$dummies.FE)),
                    filter_age = age.cur,
                    average.age = TRUE,
                    order = TRUE,
                    background_prior = T,
  ...
  ggtitle("Intervention/covariate")+
  theme(plot.title = element_text(size = 9,hjust=-1.6,face="bold"))
ggsave("./analysis/tex/plotsnew/resallR.pdf",
       width = 4,height=4.5)
ggsave("./analysis/tex/plotsnew/resallR.tif",
       width = 4,height=4.5, dpi = 800, device="tiff")
...
df.eff1 <- df.eff %>%
  group_by(m) %>%
  left_join(CovidGer::regionaldatenbank%>%
              filter(adm.level==1)%>%
              select(age,total)) %>%
...
p.weather2 <- show_total_effect(
  ...
  scale_x_date(date_labels = "%b")+
  theme(axis.title.x=element_blank())+
  scale_y_continuous(name="weather",
                     labels = function(x) paste0(x,"%"))+
  theme(
    strip.background = element_blank(),
    strip.text.y = element_blank(),
...
p.traced <- show_total_effect(average_c = TRUE,
                  ...
                  choose_age = total.age,
                  covariates = c("traced"))+
  scale_y_continuous(name="testing & tracing",
                     labels = function(x) paste0(x,"%"))+
  theme(
    strip.background = element_blank(),
    strip.text.y = element_blank(),
...
show_total_effect(
  ...
  theme(
    ...
    strip.text.y = element_blank(),
    axis.title.x=element_blank())+
  scale_y_continuous(name="changes in transmission",labels = function(x) paste0(x,"%"))
ggsave("./analysis/tex/plotsnew/changes_total.pdf",
       width = 6,height=2)
...
ggsave("./analysis/tex/plotsnew/total_allR.pdf",
       width = 6,height=2)
ggsave("./analysis/tex/plotsnew/total_allR.tif",
       width = 6,height=2, dpi = 800, device="tiff")
...
breaks.cur <- (-3:14)
### SI distribution
df.si <- InfSup::pull_mcmc_sample("SI_dist")
...
df.trans <- InfSup::pull_mcmc_sample("transmission_dist")
...
ggplot(
  rbind(
    data.frame(name = factor("basic rep. number",levels=c("basic rep. number","timing first case","number of infections"))
, value = df.R0_name$Mean),
    data.frame(name = factor("timing first case",levels=c("basic rep. number","timing first case","number of infections")),
               value = df.R0_name %>% left_join(data.frame(first = dat$Ti, name = names(dat$Ti))) |> pull(first)),
    data.frame(name = factor("number of infections",levels=c("basic rep. number","timing first case","number of infections")),
               value = df.tau$Mean)
...
effects %>%
  ...
  scale_x_discrete(label = my_labeller3)
ggsave("./analysis/tex/plotsnew/estimates_corr_aveR.pdf",
       width = 8,height=8)
...
effects %>%
  ...
  scale_y_discrete(label = my_labeller3)+
  scale_x_discrete(label = my_labeller3)
ggsave("./analysis/tex/plotsnew/estimates_corr_all_aveR.pdf",
       width = 9,height=9)
...
# effect of all interventions ---------------------------------------------


dfs <- show_total_effect(setdiff(macro$dummies.interventions,c("holidays","shops2","churches open","schools openlimited","kitas openlimited")),draws_max = 100,return.df = TRUE,average_c = T,absoluteR = TRUE,average_age = T)
dfs |> ungroup() |>
  filter(date==min(date)) |>
  group_by(iteration) |>
  summarise(effect = prod(1+draw)) |>
  ungroup() |>
  summarise(average.effect = mean(1-effect),
            low = quantile(1-effect,.025),
            up = quantile(1-effect,.975))
...
ggplot(Covid::tests,aes(x=date,y=new.tests/1000))+
  geom_point()+
  geom_line()+
...
ggsave("./analysis/tex/plotsnew/scfr_time.tif",width = 5,height=3, dpi = 800, device="tiff")
...
# + difference school and bars --------------------------------------------

data%>%
  rename(county=name) |>
  group_by(county,date)%>%
  select(county,date, macro$dummies.interventions)%>%
  filter(date > start.date,
         date<end.date1)%>%
  pivot_longer(cols = -c(date,county)) |>
  filter(name=="education"|name=="bars",value==TRUE) |>
  group_by(county,name) |>
  summarize(date=min(date)) |>
  ungroup() |>
  pivot_wider(names_from = "name",values_from = "date") |>
  ggplot(aes(bars,education))+
  geom_count()
...
data%>%
  ...
  group_by(date,name) |>
  summarise(ratio=mean(value)) |>
  mutate(active = ifelse(ratio==0,"nowhere",
                         ifelse(ratio==1, "everywhere", "partly")),
         active = factor(active, levels = c("nowhere","partly","everywhere")),
         name = my_labeller(name)) |>
  group_by(name) |>
  mutate(first = min(date[active!="nowhere"])) |>
  ungroup() |>
  mutate(name = fct_reorder(name,first,.desc = T)) |>
  ggplot(aes(x=date,y=name,col=active,alpha=active))+
  geom_point()+
  theme(axis.title = element_blank())
ggsave("./analysis/tex/plotsnew/interventions_timeline_partly.pdf",
       width = 6,height=4)
ggsave("./analysis/tex/plotsnew/interventions_timeline_partly.tif",
       dpi = 800, device="tiff")
...
ggsave("./analysis/tex/plotsnew/interventions_by_unitR.jpeg",
       width = 10,height=10)
...
ggplot(data%>%
  ...
  theme(axis.title = element_blank(),
        ...
        legend.title=element_blank())+
  scale_color_discrete(labels=c("inactive","active"))
ggsave("./analysis/tex/plotsnew/interventions_by_nameR.jpeg",
       width = 10,height=10)
...
ggsave("./analysis/tex/plotsnew/timelineR.pdf",
       width = 7,height=3)
...
cur.dat <- data %>% filter(date>= start.date,
                           date<= end.date1)%>%
  # select(name,date,all_of(cov.main))%>% # important cov only
  select(name,date,all_of(c(macro$cov,setdiff(macro$dummies,macro$dummies.FE))))%>%
  pivot_longer(-c(name,date),names_to = "cov")%>%
  group_by(cov)%>%
  summarise(
    ...
    max = max(value),
    vart = max(0, InfSup::compute_variation_in(value,date)),
    varl = max(0,InfSup::compute_variation_in(value,name)),
